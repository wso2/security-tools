            ...
            <div class="jumbotron"
                ...
                <form action="/scan-manager/submit-scan" method="post"
                    ...
                    <input type="hidden" class="form-control" id="scanType"
                           name="scanType" value="${scannerData.scannerType}">

                    <c:if test="${scannerData.fields.size() > 0}">
                    <div class="form-group">
                        <c:forEach items="${scannerData.fields}" var="field">
                            <label for="${field.fieldId}">${field.displayName}</label>
                            <input type="${field.type}" class="form-control"
                                   id="${field.fieldId}"
                                   name="${field.fieldId}" required>
                            <c:if test="${field.hasDefault}">
                                <input type="checkbox" name="${field.fieldId}" value="true"
                                       onclick="document.getElementById('${field.fieldId}').disabled=this.checked;">
                                use default<br>
                            </c:if>
