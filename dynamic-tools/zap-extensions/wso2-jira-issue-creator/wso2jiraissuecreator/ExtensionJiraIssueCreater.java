...
public class ExtensionJiraIssueCreater extends ExtensionAdaptor {
    ...
    private AbstractPanel getStatusPanel() {
        if (statusPanel == null) {
            ...
            statusPanel.setName(Constant.messages.getString(IssueCreatorConstants.PREFIX + ".panel.title"));
            statusPanel.setIcon(ICON);
            JTextPane pane = new JTextPane();
            pane.setEditable(false);
            pane.setFont(new Font("Dialog", Font.PLAIN, 12));
            pane.setContentType("text/html");
            pane.setText(Constant.messages.getString(IssueCreatorConstants.PREFIX + ".panel.msg"));
    ...
    private ZapMenuItem getMenuExample() {
        if (menuExample == null) {
            menuExample = new ZapMenuItem(IssueCreatorConstants.PREFIX + ".topmenu.report.title");
                    ...
                    if (cred_file.exists()) { //if file exists read from file
                        ...
                        try {
                            ...
                            View.getSingleton().showWarningDialog(
                                    "Wrong Credentials! Please login to your jira account and retry!!");
    ...
    public void createJiraIssues(String jiraBaseURL, String jiraUserName, String jiraPassword, String projectKey,
            String asssignee, String high, String medium, String low,String issueLabel, String filterIssuesByResourceType) {
        ...
        try {
            ...
            if (high.equals("1") || medium.equals("1") || low.equals("1")) {
                issueList = xmlParser.parseXmlDoc(project_key, asssignee, stringToBool(high), stringToBool(medium),
                        stringToBool(low), issueLabel,stringToBool(filterIssuesByResourceType));
                ...
                if (issueCount != 0) { //proceed if the issue count is > 1
                    for (int i = 0; i < issueCount; i++) { //create Issues in jira
                        ...
                        if (xmlParser.checkForIssueExistence(issueList[i],
                            ...
                            System.out.println("This is an existing issue");
